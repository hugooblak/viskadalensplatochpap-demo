...
if ( 'start' === $npl_variant ) :
	...
	$npl_button     = ! empty( $attributes['buttonText'] ) ? $attributes['buttonText'] : __( 'Räkna på mitt tak', 'viskadalensplatochpap-leads' );
	?>
	<div <?php echo $npl_wrapper; // phpcs:ignore WordPress.Security.EscapeOutput ?>>
		<form class="npl-start" method="get" action="<?php echo esc_url( $npl_action ); ?>" novalidate data-npl-start>
			<label class="npl-label" for="<?php echo esc_attr( $npl_uid ); ?>-zip"><?php esc_html_e( 'Ditt postnummer', 'viskadalensplatochpap-leads' ); ?></label>
			...
			<p class="npl-fineprint"><?php esc_html_e( 'Steg 1 av 4 · dina uppgifter lämnas aldrig vidare', 'viskadalensplatochpap-leads' ); ?></p>
		</form>
	</div>
	<?php
	return;
...
$npl_steps = array(
	1 => __( 'Var ligger huset?', 'viskadalensplatochpap-leads' ),
	2 => __( 'Vad behöver du hjälp med?', 'viskadalensplatochpap-leads' ),
	3 => __( 'När vill du ha det gjort?', 'viskadalensplatochpap-leads' ),
	4 => __( 'Vart skickar vi offerten?', 'viskadalensplatochpap-leads' ),
);
?>
	...
	<form class="npl-steps" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" novalidate data-npl-steps data-total="4" data-start="<?php echo (int) $npl_start; ?>">
		...
		<?php if ( $npl_errors ) : ?>
			<div class="npl-summary" role="alert" tabindex="-1" data-npl-summary>
				<p><strong><?php esc_html_e( 'Kolla de här svaren:', 'viskadalensplatochpap-leads' ); ?></strong></p>
				<ul>
		...
		<div class="npl-progress" data-npl-progress hidden>
			<p class="npl-progress-text" aria-live="polite" data-npl-progress-text><?php echo esc_html( sprintf( /* translators: %d: step number */ __( 'Steg %d av 4', 'viskadalensplatochpap-leads' ), $npl_start ) ); ?></p>
		...
		<fieldset class="npl-step" data-step="1" data-name="zip" tabindex="-1">
			...
			<label class="npl-label" for="<?php echo esc_attr( $npl_uid ); ?>-zip"><?php esc_html_e( 'Postnummer', 'viskadalensplatochpap-leads' ); ?></label>
		...
		foreach ( array( 2 => 'service', 3 => 'timeline' ) as $npl_n => $npl_field ) :
			...
			<fieldset class="npl-step" data-step="<?php echo (int) $npl_n; ?>" data-name="<?php echo esc_attr( $npl_field ); ?>" tabindex="-1" id="<?php echo esc_attr( $npl_uid . '-' . $npl_field ); ?>" aria-describedby="<?php echo esc_attr( $npl_uid . '-' . $npl_field ); ?>-err"<?php echo isset( $npl_errors[ $npl_field ] ) ? ' aria-invalid="true"' : ''; ?>>
				...
				<p class="npl-hint npl-auto-hint"><?php esc_html_e( 'När du väljer går du vidare till nästa fråga.', 'viskadalensplatochpap-leads' ); ?></p>
		...
		<fieldset class="npl-step" data-step="4" data-name="contact" tabindex="-1">
			...
			<?php
			$npl_contact = array(
				'name'  => array( __( 'Förnamn', 'viskadalensplatochpap-leads' ), 'text', 'given-name', '' ),
				'phone' => array( __( 'Mobilnummer', 'viskadalensplatochpap-leads' ), 'tel', 'tel-national', __( 'För att boka tid för besiktningen.', 'viskadalensplatochpap-leads' ) ),
				'email' => array( __( 'E-post', 'viskadalensplatochpap-leads' ), 'email', 'email', __( 'Hit skickar vi den skriftliga offerten.', 'viskadalensplatochpap-leads' ) ),
			);
		...
		<div class="npl-nav">
			<button type="button" class="npl-back" data-npl-back hidden><span aria-hidden="true">←</span> <?php esc_html_e( 'Back', 'viskadalensplatochpap-leads' ); ?></button>
			<button type="button" class="npl-btn wp-element-button" data-npl-next hidden><?php esc_html_e( 'Next', 'viskadalensplatochpap-leads' ); ?> <span aria-hidden="true">→</span></button>
			<button type="submit" class="npl-btn wp-element-button" data-npl-submit><?php esc_html_e( 'Skicka min förfrågan', 'viskadalensplatochpap-leads' ); ?></button>
		</div>

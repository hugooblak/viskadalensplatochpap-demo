...
add_filter(
	...
	function ( $exporters ) {
		$exporters['viskadalensplatochpap-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'viskadalensplatochpap-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'viskadalensplatochpap-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'viskadalensplatochpap-leads',
						'group_label' => __( 'Quote requests', 'viskadalensplatochpap-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['viskadalensplatochpap-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'viskadalensplatochpap-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {

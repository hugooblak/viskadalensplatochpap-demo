...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'viskadalensplatochpap-leads' ),
					'singular_name' => __( 'Lead', 'viskadalensplatochpap-leads' ),
					'edit_item'     => __( 'Förfrågan', 'viskadalensplatochpap-leads' ),
					'search_items'  => __( 'Search leads', 'viskadalensplatochpap-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'viskadalensplatochpap-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'viskadalensplatochpap-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'viskadalensplatochpap-leads' ),
			'npl_phone'    => __( 'Phone', 'viskadalensplatochpap-leads' ),
			'npl_service'  => __( 'Needs', 'viskadalensplatochpap-leads' ),
			'npl_timeline' => __( 'When', 'viskadalensplatochpap-leads' ),
			'npl_zip'      => __( 'ZIP', 'viskadalensplatochpap-leads' ),
			'npl_source'   => __( 'Source', 'viskadalensplatochpap-leads' ),
			'npl_status'   => __( 'Status', 'viskadalensplatochpap-leads' ),
			'date'         => __( 'Received', 'viskadalensplatochpap-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'viskadalensplatochpap-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'viskadalensplatochpap-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'viskadalensplatochpap-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'viskadalensplatochpap-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'viskadalensplatochpap-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'viskadalensplatochpap-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'viskadalensplatochpap-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'viskadalensplatochpap-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'viskadalensplatochpap-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'viskadalensplatochpap-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'viskadalensplatochpap-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'viskadalensplatochpap-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';

 ...
 * or photo. They call vi_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Viskadalens Plåt och Papptak AB.
 *
 ...
 * @return string
 */
function vi_sample_note() {
	return vi_lead( 'ar_riktig' ) ? '' : ' <span class="vi-sample">(platshållare)</span>';
}
 ...
 * @return string File name for vi_lead_img().
 */
function vi_lead_bild( $i ) {
	$bilder = (array) vi_lead( 'bilder', array() );
 ...
 * @return string
 */
function vi_ring_label() {
	$namn = vi_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, vi_lead( 'telefon' ) ) : 'Ring ' . vi_lead( 'telefon' );
}

<?php
/**
 * Viskadalens Plåt och Papptak AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Viskadalens Plåt och Papptak AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through vi_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'viskadalensplatochpap', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'viskadalensplatochpap' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'viskadalensplatochpap' ) ) );

		register_block_pattern_category( 'viskadalensplatochpap-sections', array( 'label' => __( 'Viskadalens Plåt och Papptak AB sections', 'viskadalensplatochpap' ) ) );
		register_block_pattern_category( 'viskadalensplatochpap-pages', array( 'label' => __( 'Viskadalens Plåt och Papptak AB page layouts', 'viskadalensplatochpap' ) ) );
	}
);
